...
TeamWork helps users run agents, skills, and MCP. It is an open-source alternative to Claude Cowork/Codex as a desktop app.

## What TeamWork Is

TeamWork is a practical control surface for agentic work:

* Run local and remote agent workflows from one place.
* Use OpenCode capabilities directly through TeamWork.
* Compose desktop app, server, and messaging connectors without lock-in.
* Treat the TeamWork app as a client of the TeamWork server API surface.
* Connect to hosted workers through a simple user flow: `Add a worker` -> `Connect remote`.

## Core Philosophy

* **Local-first, cloud-ready**: TeamWork runs on your machine in one click and can connect to cloud workflows when needed.
* **Server-consumption first**: the app should consume TeamWork server surfaces (self-hosted or hosted), not invent parallel behavior.
* **Composable**: use the desktop app, WhatsApp/Slack/Telegram connectors, or server mode based on the task.
* **Ejectable**: TeamWork is powered by OpenCode, so anything OpenCode can do is available in TeamWork, even before a dedicated UI exists.
...
TeamWork now has three production-grade ways to run the same product surface:

1. **Desktop-hosted app/server**
   - TeamWork app runs locally and can host server functionality on-device.
2. **CLI-hosted server (teamwork-orchestrator)**
   - TeamWork server surfaces can be provided by the orchestrator/CLI on a trusted machine.
3. **Hosted TeamWork Cloud server**
   - TeamWork-hosted infrastructure provisions workers and exposes the same remote-connect semantics.
...
* **Web parity**: anything that mutates `.opencode/` should be expressible via the TeamWork server API; Tauri-only filesystem calls are a fallback for host mode, not a separate capability set.
...
* **Provider-neutral control**: expose app actions through TeamWork-owned control surfaces first; provider-specific controllers should drive those surfaces rather than hardwiring provider logic into the app UI.
...
1. `Target repo: <path>` (for example: `_repos/teamwork`)
...
TeamWork aims to be a **living system**: agents, skills, commands, and config are hot-reloadable while sessions are running. This enables agents to create new skills or update their own configuration and have changes take effect immediately, without tearing down active sessions.

Design principles for hot reload:

* **Conservative triggers**: only reload when a file that OpenCode reads at startup actually changes inside `.opencode/` or `opencode.json`. Ignore metadata files like `teamwork.json`, `.DS_Store`, etc.
...
* **Auto-reload setting**: each workspace can opt into automatic reload via `.opencode/teamwork.json` (`reload.auto`). When enabled, the engine reloads automatically once queued signals are ready and no sessions are active.
...
* Use `DESIGN-LANGUAGE.md` as the default visual reference for TeamWork app and landing work.
* For TeamWork session-surface details, also reference `packages/docs/orbita-layout-style.mdx`.
...
* If you change `apps/server/src`, rebuild the TeamWork server binary (`pnpm --filter teamwork-server build:bin`) because `teamwork` (teamwork-orchestrator) runs the compiled server, not the TS sources.
...
TeamWork integrates with OpenCode via:
...
This captures TeamWork’s preferred reactivity + UI state patterns (avoid global `busy()` deadlocks; use scoped async state).

## Skill: Trigger a Release

TeamWork releases are built by GitHub Actions (`Release App`). A release is triggered by pushing a `v*` tag (e.g. `v0.1.6`).
`Release App` can also publish teamwork-orchestrator sidecars and npm packages when enabled via workflow inputs or repo vars (`RELEASE_PUBLISH_SIDECARS`, `RELEASE_PUBLISH_NPM`).
...
* `apps/orchestrator/package.json` (`version`, publishes as `teamwork-orchestrator`)
...
* `gh workflow run "Release App" --repo SamirTafesh/TeamWork -f tag=vX.Y.Z`

### Verify

* Runs: `gh run list --repo SamirTafesh/TeamWork --workflow "Release App" --limit 5`
* Release: `gh release view vX.Y.Z --repo SamirTafesh/TeamWork`

Confirm the DMG assets are attached and versioned correctly.

## Skill: Publish teamwork-orchestrator (npm)

This is usually covered by `Release App` when `publish_sidecars` + `publish_npm` are enabled. Use `.opencode/skills/teamwork-orchestrator-npm-publish/SKILL.md` for manual recovery or one-off publishing.
...
4.  Build and upload sidecar assets for the same version tag:
    ...
    * `gh release create teamwork-orchestrator-vX.Y.Z apps/orchestrator/dist/sidecars/* --repo SamirTafesh/TeamWork`
5.  Publish:
