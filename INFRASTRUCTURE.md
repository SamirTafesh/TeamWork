# TeamWork Infrastructure Principles

TeamWork is an experience layer. `opencode` is the engine. This document defines how infrastructure is built so every component is usable on its own, composable as a sidecar, and easy to automate.
...
* The TeamWork UI may wrap these, but never replace or lock them out.
...
* OpenCode remains the engine; TeamWork adds a thin config + UX layer.
...
* TeamWork never replaces the CLI; it only connects to the engine.

### TeamWork Server

* Runs standalone via `teamwork-server` CLI.
* Provides filesystem-backed config surfaces (skills, plugins, MCP, commands).
* Sidecar lifecycle is described in `packages/app/pr/teamwork-server.md`.
* Can also be consumed as a hosted TeamWork Cloud control surface for remote worker lifecycle.

### TeamWork Cloud Control Plane

* Hosted deployment of TeamWork server capabilities for worker provisioning and remote connect.
...
* Must be able to use TeamWork server for config and approvals.
...
* `PRINCIPLES.md`
* `ARCHITECTURE.md`
* `packages/app/pr/teamwork-server.md`
