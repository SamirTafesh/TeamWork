const ENV_FEEDBACK_URL = String(import.meta.env.VITE_TEAMWORK_FEEDBACK_URL ?? "").trim();
const ENV_APP_VERSION = String(import.meta.env.VITE_TEAMWORK_APP_VERSION ?? "").trim();

export const DEFAULT_FEEDBACK_URL =
  ENV_FEEDBACK_URL || "https://teamworklabs.com/feedback";

type FeedbackUrlOptions = {
  entrypoint: string;
  deployment?: string | null;
  appVersion?: string | null;
  teamworkServerVersion?: string | null;
...
export function buildFeedbackUrl(options: FeedbackUrlOptions): string {
  ...
  url.searchParams.set("source", "teamwork-app");
  ...
  const entries = {
    ...
    teamworkServerVersion: options.teamworkServerVersion?.trim() ?? "",
