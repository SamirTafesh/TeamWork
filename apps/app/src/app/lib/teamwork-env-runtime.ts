const PENDING_CHANGES_KEY = "teamwork.settings.environment.pendingChanges";
...
export function buildTeamworkEnvRuntimeKey(input: {
  baseUrl?: string | null;
  pid?: number | null;
  port?: number | null;
}): string | undefined {
  ...
  return `${baseUrl || "teamwork"}::${runtime || "runtime"}`;
}

export function readTeamworkEnvPendingChanges(runtimeKey?: string | null): boolean {
...
export function writeTeamworkEnvPendingChanges(value: boolean, runtimeKey?: string | null): void {
  const localStorage = getStorage("localStorage");
  const sessionStorage = getStorage("sessionStorage");
  try {
    if (value) {
