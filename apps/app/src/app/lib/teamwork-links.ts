...
import { normalizeTeamworkServerUrl } from "./teamwork-server";
...
export function parseRemoteConnectDeepLink(rawUrl: string): RemoteWorkspaceDefaults | null {
  ...
  const hostUrlRaw = url.searchParams.get("teamworkHostUrl") ?? url.searchParams.get("teamworkUrl") ?? "";
  const tokenRaw = url.searchParams.get("teamworkToken") ?? url.searchParams.get("accessToken") ?? "";
  const normalizedHostUrl = normalizeTeamworkServerUrl(hostUrlRaw);
  ...
  return {
    teamworkHostUrl: normalizedHostUrl,
    teamworkToken: token,
    directory: null,
...
function normalizeDebugDeepLinkInput(rawValue: string): string {
  ...
  const directMatch = trimmed.match(/(?:teamwork-dev|teamwork|https?):\/\/[^\s"'<>]+/i);
  if (directMatch) return directMatch[0];

  const bareShareMatch = trimmed.match(/share\.teamwork(?:labs\.com|\.software)\/b\/[^\s"'<>]+/i);
...
export function parseDebugDeepLinkInput(rawValue: string):
  ...
  const shareIdMatch = normalized.match(/share\.teamwork(?:labs\.com|\.software)\/b\/([^\s/?#"'<>]+)/i);
  if (shareIdMatch?.[1]) {
    return {
      kind: "bundle",
      link: {
        bundleUrl: `https://share.teamworklabs.com/b/${shareIdMatch[1]}`,
        intent: "new_worker",
      },
    };
  }
