...
export type TeamworkServerCapabilities = {
  skills: { read: boolean; write: boolean; source: "teamwork" | "opencode" };
  hub?: {
    skills?: {
      read: boolean;
      install: boolean;
...
export type TeamworkServerStatus = "connected" | "disconnected" | "limited";

export type TeamworkServerDiagnostics = {
  ok: boolean;
  version: string;
  uptimeMs: number;
  readOnly: boolean;
  ...
  workspace: TeamworkWorkspaceInfo | null;
  authorizedRoots: string[];
  server: { host: string; port: number; configPath?: string | null };
  tokenSource: { client: string; host: string };
};

export type TeamworkRuntimeServiceName = "teamwork-server" | "opencode";

export type TeamworkRuntimeServiceSnapshot = {
  name: TeamworkRuntimeServiceName;
  enabled: boolean;
  running: boolean;
...
export type TeamworkRuntimeSnapshot = {
  ok: boolean;
  orchestrator?: {
    version: string;
    startedAt: number;
  ...
  upgrade?: {
    ...
    services: TeamworkRuntimeServiceName[];
  };
  services: TeamworkRuntimeServiceSnapshot[];
};

export type TeamworkServerSettings = {
  urlOverride?: string;
  portOverride?: number;
  token?: string;
  hostToken?: string;
...
export type TeamworkWorkspaceInfo = WorkspaceInfo & {
  opencode?: {
    baseUrl?: string;
    directory?: string;
    username?: string;
...
export type TeamworkWorkspaceList = {
  items: TeamworkWorkspaceInfo[];
  workspaces?: WorkspaceInfo[];
  activeId?: string | null;
};

export type TeamworkSessionMessage = {
  info: Message;
  parts: Part[];
};

export type TeamworkSessionSnapshot = {
  session: Session;
  messages: TeamworkSessionMessage[];
  todos: Todo[];
  status:
    | { type: "idle" }
    | { type: "busy" }
...
export type TeamworkPluginItem = {
  spec: string;
...
export type TeamworkSkillContent = {
  item: TeamworkSkillItem;
  content: string;
};

export type TeamworkHubSkillItem = {
  name: string;
  description: string;
  trigger?: string;
  source: {
...
export type TeamworkHubRepo = {
  owner?: string;
  repo?: string;
  ref?: string;
};

export type TeamworkWorkspaceFileContent = {
  path: string;
  content: string;
  bytes: number;
  updatedAt: number;
...
export type TeamworkWorkspaceFileWriteResult = {
  ok: boolean;
  path: string;
  bytes: number;
  updatedAt: number;
...
export type TeamworkCommandItem = {
  name: string;
  description?: string;
  template: string;
  agent?: string;
...
export type TeamworkWorkspaceExport = {
  workspaceId: string;
  exportedAt: number;
...
export type TeamworkWorkspaceImportChange = {
  kind: "opencode" | "teamwork" | "skill" | "command" | "file";
  action: "create" | "update" | "replace" | "delete" | "unchanged";
  label: string;
  path: string;
};

export type TeamworkWorkspaceImportPreview = {
  fingerprint: string;
  summary: {
    total: number;
    create: number;
  ...
  changes: TeamworkWorkspaceImportChange[];
};

export type TeamworkWorkspaceExportSensitiveMode = "auto" | "include" | "exclude";

export type TeamworkWorkspaceExportWarning = {
  id: string;
  label: string;
  detail: string;
};

export type TeamworkBlueprintSessionsMaterializeResult = {
  ok: boolean;
...
export type TeamworkArtifactItem = {
  id: string;
  name?: string;
  path?: string;
  size?: number;
...
export type TeamworkArtifactList = {
  items: TeamworkArtifactItem[];
};

export type TeamworkInboxItem = {
  id: string;
  name?: string;
  path?: string;
  size?: number;
...
export type TeamworkInboxList = {
  items: TeamworkInboxItem[];
};

export type TeamworkInboxUploadResult = {
  ok: boolean;
  path: string;
  bytes: number;
};
...
export type TeamworkAuditEntry = {
  id: string;
  workspaceId: string;
  actor: TeamworkActor;
  action: string;
...
export type TeamworkReloadTrigger = {
...
export type TeamworkReloadEvent = {
  id: string;
  seq: number;
  workspaceId: string;
  reason: "plugins" | "skills" | "mcp" | "config" | "agents" | "commands";
  trigger?: TeamworkReloadTrigger;
  timestamp: number;
};
...
export const DEFAULT_TEAMWORK_SERVER_PORT = 8787;

const STORAGE_URL_OVERRIDE = "teamwork.server.urlOverride";
const STORAGE_PORT_OVERRIDE = "teamwork.server.port";
const STORAGE_TOKEN = "teamwork.server.token";
const STORAGE_HOST_TOKEN = "teamwork.server.hostToken";
const STORAGE_REMOTE_ACCESS = "teamwork.server.remoteAccessEnabled";

export function normalizeTeamworkServerUrl(input: string) {
...
export function isLoopbackTeamworkServerUrl(input: string) {
  const normalized = normalizeTeamworkServerUrl(input) ?? "";
...
export function parseTeamworkWorkspaceIdFromUrl(input: string) {
  const normalized = normalizeTeamworkServerUrl(input) ?? "";
...
export function buildTeamworkWorkspaceBaseUrl(hostUrl: string, workspaceId?: string | null) {
  const normalized = normalizeTeamworkServerUrl(hostUrl) ?? "";
  if (!normalized) return null;
...
const TEAMWORK_INVITE_PARAM_URL = "ow_url";
const TEAMWORK_INVITE_PARAM_TOKEN = "ow_token";
const TEAMWORK_INVITE_PARAM_STARTUP = "ow_startup";
const TEAMWORK_INVITE_PARAM_AUTO_CONNECT = "ow_auto_connect";

export type TeamworkConnectInvite = {
  url: string;
  token?: string;
  startup?: "server";
  autoConnect?: boolean;
...
export function readTeamworkConnectInviteFromSearch(input: string | URLSearchParams) {
  const search =
    typeof input === "string"
      ? new URLSearchParams(input.startsWith("?") ? input.slice(1) : input)
      : input;

  const rawUrl = search.get(TEAMWORK_INVITE_PARAM_URL)?.trim() ?? "";
  const url = normalizeTeamworkServerUrl(rawUrl);
  if (!url) return null;

  const token = search.get(TEAMWORK_INVITE_PARAM_TOKEN)?.trim() ?? "";
  const startupRaw = search.get(TEAMWORK_INVITE_PARAM_STARTUP)?.trim() ?? "";
  const startup = startupRaw === "server" ? "server" : undefined;
  const autoConnect = search.get(TEAMWORK_INVITE_PARAM_AUTO_CONNECT)?.trim() === "1";
  ...
  } satisfies TeamworkConnectInvite;
}

export function stripTeamworkConnectInviteFromUrl(input: string) {
  try {
    const url = new URL(input);
    url.searchParams.delete(TEAMWORK_INVITE_PARAM_URL);
    url.searchParams.delete(TEAMWORK_INVITE_PARAM_TOKEN);
    url.searchParams.delete(TEAMWORK_INVITE_PARAM_STARTUP);
    url.searchParams.delete(TEAMWORK_INVITE_PARAM_AUTO_CONNECT);
    return url.toString();
  } catch {
    return input;
  }
...
export function readTeamworkServerSettings(): TeamworkServerSettings {
  if (typeof window === "undefined") return {};
  try {
    const urlOverride = normalizeTeamworkServerUrl(
...
export function writeTeamworkServerSettings(next: TeamworkServerSettings): TeamworkServerSettings {
  if (typeof window === "undefined") return next;
  try {
    const urlOverride = normalizeTeamworkServerUrl(next.urlOverride ?? "");
    ...
    return readTeamworkServerSettings();
  } catch {
    return next;
  }
}

export function hydrateTeamworkServerSettingsFromEnv() {
  if (typeof window === "undefined") return;

  const envUrl = typeof import.meta.env?.VITE_TEAMWORK_URL === "string"
    ? import.meta.env.VITE_TEAMWORK_URL.trim()
    : "";
  const envPort = typeof import.meta.env?.VITE_TEAMWORK_PORT === "string"
    ? import.meta.env.VITE_TEAMWORK_PORT.trim()
    : "";
  const envToken = typeof import.meta.env?.VITE_TEAMWORK_TOKEN === "string"
    ? import.meta.env.VITE_TEAMWORK_TOKEN.trim()
    : "";
  const envHostToken = typeof import.meta.env?.VITE_TEAMWORK_HOST_TOKEN === "string"
    ? import.meta.env.VITE_TEAMWORK_HOST_TOKEN.trim()
    : "";

  if (!envUrl && !envPort && !envToken && !envHostToken) return;

  try {
    const current = readTeamworkServerSettings();
    const next: TeamworkServerSettings = { ...current };
    let changed = false;

    if (!current.urlOverride && envUrl) {
      next.urlOverride = normalizeTeamworkServerUrl(envUrl) ?? undefined;
      changed = true;
    }
...
export function clearTeamworkServerSettings() {
  if (typeof window === "undefined") return;
  try {
    window.localStorage.removeItem(STORAGE_URL_OVERRIDE);
    window.localStorage.removeItem(STORAGE_PORT_OVERRIDE);
...
export class TeamworkServerError extends Error {
  status: number;
  code: string;
  details?: unknown;
...
function buildHeaders(
  ...
  if (hostToken) {
    headers["X-TeamWork-Host-Token"] = hostToken;
  }
  if (extra) {
...
const TEAMWORK_STREAM_URL_RE = /\/events(\b|\?)|\/event-stream\b|\/stream\b/;

function isStreamUrl(url: string): boolean {
  return TEAMWORK_STREAM_URL_RE.test(url);
}
...
const DEFAULT_TEAMWORK_SERVER_TIMEOUT_MS = 10_000;
...
async function requestJson<T>(
  ...
  const response = await fetchWithTimeout(
    ...
    options.timeoutMs ?? DEFAULT_TEAMWORK_SERVER_TIMEOUT_MS,
  );
  ...
  if (!response.ok) {
    ...
    throw new TeamworkServerError(response.status, code, message, json?.details);
  }
...
export function createTeamworkServerClient(options: { baseUrl: string; token?: string; hostToken?: string }) {
  ...
  return {
    ...
    runtimeVersions: () =>
      requestJson<TeamworkRuntimeSnapshot>(baseUrl, "/runtime/versions", { token, hostToken, timeoutMs: timeouts.status }),
    status: () => requestJson<TeamworkServerDiagnostics>(baseUrl, "/status", { token, hostToken, timeoutMs: timeouts.status }),
    capabilities: () => requestJson<TeamworkServerCapabilities>(baseUrl, "/capabilities", { token, hostToken, timeoutMs: timeouts.capabilities }),
    listWorkspaces: () => requestJson<TeamworkWorkspaceList>(baseUrl, "/workspaces", { token, hostToken, timeoutMs: timeouts.listWorkspaces }),
    ...
    activateWorkspace: (workspaceId: string) =>
      requestJson<{ activeId: string; workspace: TeamworkWorkspaceInfo }>(
        baseUrl,
        `/workspaces/${encodeURIComponent(workspaceId)}/activate`,
        { token, hostToken, method: "POST", timeoutMs: timeouts.activateWorkspace },
      ),
    deleteWorkspace: (workspaceId: string) =>
      requestJson<{ ok: boolean; deleted: boolean; persisted: boolean; activeId: string | null; items: TeamworkWorkspaceInfo[]; workspaces?: WorkspaceInfo[] }>(
        baseUrl,
    ...
    getSessionMessages: (workspaceId: string, sessionId: string, options?: { limit?: number }) => {
      ...
      if (typeof options?.limit === "number") query.set("limit", String(options.limit));
      const suffix = query.size ? `?${query.toString()}` : "";
      return requestJson<{ items: TeamworkSessionMessage[] }>(
        baseUrl,
    ...
    getSessionSnapshot: (workspaceId: string, sessionId: string, options?: { limit?: number }) => {
      ...
      if (typeof options?.limit === "number") query.set("limit", String(options.limit));
      const suffix = query.size ? `?${query.toString()}` : "";
      return requestJson<{ item: TeamworkSessionSnapshot }>(
        baseUrl,
    ...
    exportWorkspace: (
      workspaceId: string,
      options?: { sensitiveMode?: TeamworkWorkspaceExportSensitiveMode },
    ) => {
      const query = new URLSearchParams();
      ...
      const suffix = query.size ? `?${query.toString()}` : "";
      return requestJson<TeamworkWorkspaceExport>(baseUrl, `/workspace/${encodeURIComponent(workspaceId)}/export${suffix}`, {
        token,
        hostToken,
    ...
    importWorkspace: (workspaceId: string, payload: Record<string, unknown>) =>
      requestJson<{ ok: boolean; preview?: TeamworkWorkspaceImportPreview }>(baseUrl, `/workspace/${encodeURIComponent(workspaceId)}/import`, {
        token,
        hostToken,
        method: "POST",
        body: payload,
    ...
    previewWorkspaceImport: (workspaceId: string, payload: Record<string, unknown>) =>
      requestJson<TeamworkWorkspaceImportPreview>(
        baseUrl,
    ...
    materializeBlueprintSessions: (workspaceId: string) =>
      requestJson<TeamworkBlueprintSessionsMaterializeResult>(
        baseUrl,
    ...
    getConfig: (workspaceId: string) =>
      requestJson<{ opencode: Record<string, unknown>; teamwork: Record<string, unknown>; updatedAt?: number | null }>(
        baseUrl,
        `/workspace/${workspaceId}/config`,
        { token, hostToken, timeoutMs: timeouts.config },
      ),
    patchConfig: (workspaceId: string, payload: { opencode?: Record<string, unknown>; teamwork?: Record<string, unknown> }) =>
    ...
    listPlugins: (workspaceId: string, options?: { includeGlobal?: boolean }) => {
      ...
      return requestJson<{ items: TeamworkPluginItem[]; loadOrder: string[] }>(
        baseUrl,
    ...
    addPlugin: (workspaceId: string, spec: string) =>
      requestJson<{ items: TeamworkPluginItem[]; loadOrder: string[] }>(
        baseUrl,
    ...
    listSkills: (workspaceId: string, options?: { includeGlobal?: boolean }) => {
      const query = options?.includeGlobal ? "?includeGlobal=true" : "";
      return requestJson<{ items: TeamworkSkillItem[] }>(
        baseUrl,
        `/workspace/${workspaceId}/skills${query}`,
        { token, hostToken },
      );
    },
    listHubSkills: (options?: { repo?: TeamworkHubRepo }) => {
      ...
      return requestJson<{ items: TeamworkHubSkillItem[] }>(baseUrl, `/hub/skills${query}`, {
        token,
        hostToken,
      });
    },
    ...
    getSkill: (workspaceId: string, name: string, options?: { includeGlobal?: boolean }) => {
      const query = options?.includeGlobal ? "?includeGlobal=true" : "";
      return requestJson<TeamworkSkillContent>(
        baseUrl,
    ...
    upsertSkill: (workspaceId: string, payload: { name: string; content: string; description?: string }) =>
      requestJson<TeamworkSkillItem>(baseUrl, `/workspace/${workspaceId}/skills`, {
        token,
        hostToken,
        method: "POST",
        body: payload,
    ...
    listMcp: (workspaceId: string) =>
      requestJson<{ items: TeamworkMcpItem[] }>(baseUrl, `/workspace/${workspaceId}/mcp`, { token, hostToken }),
    addMcp: (workspaceId: string, payload: { name: string; config: Record<string, unknown> }) =>
      requestJson<{ items: TeamworkMcpItem[] }>(baseUrl, `/workspace/${workspaceId}/mcp`, {
        token,
        hostToken,
        method: "POST",
        body: payload,
    ...
    removeMcp: (workspaceId: string, name: string) =>
      requestJson<{ items: TeamworkMcpItem[] }>(baseUrl, `/workspace/${workspaceId}/mcp/${encodeURIComponent(name)}`, {
        token,
        hostToken,
        method: "DELETE",
      }),
    setMcpEnabled: (workspaceId: string, name: string, enabled: boolean) =>
      requestJson<{ items: TeamworkMcpItem[] }>(
        baseUrl,
    ...
    upsertCommand: (
      ...
      requestJson<{ items: TeamworkCommandItem[] }>(baseUrl, `/workspace/${workspaceId}/commands`, {
        token,
        hostToken,
        method: "POST",
        body: payload,
    ...
    uploadInbox: async (workspaceId: string, file: File, options?: { path?: string }) => {
      ...
      if (body) {
        try {
          const parsed = JSON.parse(body) as Partial<TeamworkInboxUploadResult>;
          if (typeof parsed.path === "string" && parsed.path.trim()) {
            return {
              ok: parsed.ok ?? true,
              path: parsed.path.trim(),
              bytes: typeof parsed.bytes === "number" ? parsed.bytes : file.size,
            } satisfies TeamworkInboxUploadResult;
          }
        } catch {
    ...
    listInbox: (workspaceId: string) =>
      requestJson<TeamworkInboxList>(baseUrl, `/workspace/${encodeURIComponent(workspaceId)}/inbox`, {
        token,
        hostToken,
      }),
    ...
    readWorkspaceFile: (workspaceId: string, path: string) =>
      requestJson<TeamworkWorkspaceFileContent>(
        baseUrl,
    ...
    writeWorkspaceFile: (
      ...
      requestJson<TeamworkWorkspaceFileWriteResult>(
        baseUrl,
    ...
    listArtifacts: (workspaceId: string) =>
      requestJson<TeamworkArtifactList>(baseUrl, `/workspace/${encodeURIComponent(workspaceId)}/artifacts`, {
        token,
        hostToken,
      }),
...
export type TeamworkServerClient = ReturnType<typeof createTeamworkServerClient>;
