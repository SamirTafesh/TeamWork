...
import { relaunchDesktopApp, resetTeamworkState } from "../../app/lib/desktop";
import {
...
export type ResetState = {
  ...
  resetModalMode: ResetTeamworkMode;
...
export type SystemStateControls = {
  ...
  reset: ResetState;
  openResetModal: (mode: ResetTeamworkMode) => void;
...
function clearTeamworkLocalStorage(mode: ResetTeamworkMode) {
  ...
  try {
    ...
    for (const key of keys) {
      if (key.includes("teamwork")) window.localStorage.removeItem(key);
    }
    window.localStorage.removeItem("teamwork_mode_pref");
  } catch {
    // ignore
  }
}
...
export function useSystemState(
  ...
  const [resetModalMode, setResetModalMode] =
    useState<ResetTeamworkMode>("onboarding");
  ...
  const confirmReset = useCallback(async () => {
    ...
    try {
      if (isDesktopRuntime()) {
        await resetTeamworkState(resetModalMode);
      }
      clearTeamworkLocalStorage(resetModalMode);
      if (isDesktopRuntime()) {
