 ...
 *
 * Consumers read via `window.__teamwork` from a devtools console or from
...
declare global {
  ...
  var __teamwork: InspectorAPI | undefined;
}
...
function installIfNeeded() {
  ...
  Object.defineProperty(window, "__teamwork", {
    value: api,
    configurable: true,
    writable: false,
  });
...
export function recordInspectorEvent(name: string, data?: unknown) {
  installIfNeeded();
  window.__teamwork?.record(name, data);
}
