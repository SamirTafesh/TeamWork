...
import { readTeamworkServerSettings, writeTeamworkServerSettings } from "../../app/lib/teamwork-server";
...
export async function ensureDesktopLocalTeamworkConnection(
  options: EnsureDesktopLocalTeamworkOptions,
) {
  ...
  recordInspectorEvent("route.local_teamwork.ensure.start", {
    route: options.route,
  ...
  try {
    ...
    if (!info?.baseUrl) {
      throw new Error("TeamWork server did not report a base URL after activation.");
    }
    ...
    emitTeamworkSettingsChanged();

    recordInspectorEvent("route.local_teamwork.ensure.success", {
      route: options.route,
      workspaceId: workspace.id,
      workspaceRoot,
    ...
    recordInspectorEvent("route.local_teamwork.ensure.error", {
      route: options.route,
      workspaceId: workspace.id,
      workspaceRoot,
      message,
