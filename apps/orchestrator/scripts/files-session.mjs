...
async function runCli(args) {
  ...
  if (code !== 0) {
    throw new Error(stderr.trim() || `teamwork CLI failed with code ${code}`);
  }
...
const root = await mkdtemp(join(tmpdir(), "teamwork-file-session-"));
...
try {
  await waitFor(`${teamworkUrl}/health`);

  const workspaces = await fetchJson(`${teamworkUrl}/workspaces`, {
  ...
  assert.equal(diskContent, "updated by teamwork cli\n");
  ...
  console.log(JSON.stringify({ ok: true, teamworkUrl, workspaceId, sessionId }, null, 2));
} catch (error) {
  console.error(
    JSON.stringify(
      {
