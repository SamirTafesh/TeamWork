...
test("sdk generation succeeds from the server-v2 openapi document", async () => {
  const result = await runCommand(["pnpm", "--filter", "@teamwork/server-sdk", "generate"], repoDir);

  expect(result.exitCode).toBe(0);

  const sdkIndex = await Bun.file(path.join(repoDir, "packages/teamwork-server-sdk/generated/index.ts")).text();
