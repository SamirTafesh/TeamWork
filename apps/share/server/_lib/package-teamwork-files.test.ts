...
import { packageTeamworkFiles } from "./package-teamwork-files.ts";

test("packageTeamworkFiles creates a single skill bundle from skill markdown", () => {
  const result = packageTeamworkFiles({
    files: [
      {
...
test("packageTeamworkFiles accepts any single text file when frontmatter has name and description", () => {
  const result = packageTeamworkFiles({
    files: [
      {
        path: "AGENTS.md",
...
test("packageTeamworkFiles rejects content without name and description frontmatter", () => {
  assert.throws(
    () =>
      packageTeamworkFiles({
        files: [
          {
            path: "whatever.txt",
...
test("packageTeamworkFiles rejects multiple uploaded files", () => {
  assert.throws(
    () =>
      packageTeamworkFiles({
        files: [
...
test("packageTeamworkFiles rejects config json uploads", () => {
  assert.throws(
    () =>
      packageTeamworkFiles({
        files: [
...
test("packageTeamworkFiles rejects agent and config combinations", () => {
  assert.throws(
    () =>
      packageTeamworkFiles({
        files: [
