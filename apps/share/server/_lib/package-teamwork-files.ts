...
function isTeamworkConfigFile(file: NormalizedFile): boolean {
  return /(^|\/)(?:\.opencode\/)?teamwork\.jsonc?$/i.test(file.path);
}
...
function looksLikeTeamworkShape(parsed: unknown): boolean {
  if (!isRecord(parsed)) return false;

  const schema = maybeString(parsed.$schema).toLowerCase();
  return schema.includes("teamwork") && schema.includes("config");
}
...
interface ConfigSection {
  ...
  teamwork: Record<string, unknown> | null;
  config: Record<string, unknown>;
  preview: PreviewItem[];
  configCount: number;
}
...
function readConfigSection(file: NormalizedFile, warnings: string[]): ConfigSection | null {
  ...
  let teamwork: Record<string, unknown> | null = null;
  const config: Record<string, unknown> = {};
  const preview: PreviewItem[] = [];
  let configCount = 0;

  if (isTeamworkConfigFile(file)) {
    teamwork = cloneRecord(parsed);
    preview.push(buildConfigPreview(file, "TeamWork config"));
    configCount += 1;
  }
  ...
  if (!opencode && !teamwork) {
    if (looksLikeNamedMcpFile(file)) {
      ...
      preview.push(...collectOpencodePreviewItems(parsed));
      configCount += 1;
    } else if (looksLikeTeamworkShape(parsed)) {
      teamwork = cloneRecord(parsed);
      preview.push(buildConfigPreview(file, "TeamWork config"));
      configCount += 1;
  ...
  const secretHits = [
    ...
    ...redactSecrets(teamwork ?? {}, ["teamwork"]),
  ...
  if (!opencode && !teamwork && !Object.keys(config).length) {
    warnings.push(`Ignored unsupported config file: ${file.path}`);
    return null;
  }

  return { opencode, teamwork, config, preview, configCount };
}
...
export function packageTeamworkFiles(input: PackageInput): PackageResult {
  ...
  if (!summary.skills && !summary.agents && !summary.mcpServers && !summary.commands && !summary.configs && !teamwork) {
