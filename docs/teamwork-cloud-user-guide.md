# TeamWork Cloud User Guide (End-to-End)

This guide explains how to run TeamWork as a team from first sign-up to daily production use.
...
You need:
- running TeamWork Cloud deployment (web + API reachable)
...
1. Open your Cloud URL (example: `https://teamwork.aitech-services.com`).
2. Choose one method:
   - `Sign up with email`
...
TeamWork Cloud requires an active plan for organization creation/usage.

1. Continue from onboarding.
2. If redirected to checkout, complete payment.
3. Return to TeamWork after payment.
