...
description: "Create an org-managed provider in TeamWork Cloud and import it into the desktop app"
---
...
## Create the provider in TeamWork Cloud
...
<Frame>
  ![Managed LLM provider detail in TeamWork Cloud](/images/cloud-managed-llm-provider-detail.png)
</Frame>
...
TeamWork writes the imported provider into the workspace `opencode.jsonc` and stores the org credential for that workspace.

## Update or remove it later

- Use `Sync` after changing models or access in TeamWork Cloud.
