---
title: "Self-host"
description: "Run TeamWork on your own server or infrastructure."
---

Most users just use the [desktop app](https://teamworklabs.com/download). Self-hosting is for teams that want TeamWork to run on their own server, VPC, or private cloud and then connect the desktop app to it remotely.

Want hosted workspaces, team skill hubs, or shared providers without running infra? Use [TeamWork Cloud](/cloud/get-started) instead. Rolling this out across a company? [Book a call](https://calendar.app.google/86QpCENvhfEzDFLu5) about [Enterprise](/cloud/enterprise).

## What you deploy

TeamWork is split into normal services:

- **TeamWork app**: the desktop or web client people use.
...
If you only need one private remote workspace, deploy the worker runtime. If you want TeamWork Cloud-style accounts, teams, and worker creation, deploy Den web, Den controller, the worker proxy, and worker runtimes.
...
- Expose the TeamWork server/worker URL, not raw opencode.
...
For a containerized worker runtime, start from the [Docker packaging](https://github.com/SamirTafesh/TeamWork/tree/dev/packaging/docker) in the repo.
...
If you do not deploy Den, worker access is token-based with `TEAMWORK_TOKEN` and `TEAMWORK_HOST_TOKEN`.

## Minimal Working Stack

#### TeamWork Cloud and the Desktop App

TeamWork cloud is composed of two services:
...
TeamWork does not require many third-party services for the core runtime. These are optional or deployment-specific:
...
Skip the server. [**TeamWork Cloud**](/cloud/get-started) runs hosted workers for your team.
...
If you're rolling TeamWork out across a company of 150\+ employees, the orchestrator on a single VM might be harder to manage.
