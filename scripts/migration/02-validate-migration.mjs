...
async function main() {
  ...
  const workDir = join(tmpdir(), `teamwork-migrate-validate-${args.tag}`);
  ...
  const appPath = join(unzipDir, "TeamWork.app");
  if (!existsSync(appPath)) {
    console.log(`[validate] ✗ TeamWork.app not found inside zip (got: ${unzipDir})`);
  ...
  const steps = [
    ...
    "Restart, see the 'TeamWork is moving' modal, click 'Install now'.",
