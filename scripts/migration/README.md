...
| ---- | ---- | ------------------------ |
| 01   | Ready to ship the migration | Tauri users see "TeamWork is moving" prompt on next update |
...
- Pushes to `dev` or `main` refresh the rolling prerelease bucket at
  <https://github.com/SamirTafesh/TeamWork/releases/tag/electron-preview-latest>.
...
- The install handoff requires a pasted Electron artifact URL plus two explicit
  ...
  `TeamWork.app.migrate-bak` for rollback.
...
3. Ask testers to download the matching platform artifact:
   - macOS Apple Silicon: `teamwork-mac-arm64-*.dmg` or `.zip`
   - macOS Intel: `teamwork-mac-x64-*.dmg` or `.zip`
   - Windows: `teamwork-win-x64-*.exe`
   - Linux: `teamwork-linux-x64-*.AppImage` or `.tar.gz`
...
node scripts/migration/01-cut-migration-release.mjs --version 0.12.0 \
  --mac-url 'https://github.com/SamirTafesh/TeamWork/releases/download/v0.12.0/TeamWork-darwin-arm64-0.12.0-mac.zip' \
...
3. Creates a release-config fragment at
   ...
   `VITE_TEAMWORK_MIGRATION_RELEASE=1` and the per-platform download
...
- Users still on Tauri: the migrate prompt is gated on
  `VITE_TEAMWORK_MIGRATION_RELEASE=1` at build time. Re-cut the
...
- Users mid-migration: the Rust `migrate_to_electron` command keeps
  the previous `.app` at `TeamWork.app.migrate-bak`. Instruct users
  to `mv TeamWork.app.migrate-bak TeamWork.app` if the Electron
  launch fails.
