#!/usr/bin/env bash
#
# teamwork-debug.sh — one-stop observability + lifecycle control for the
# TeamWork dev stack.
#
# Subcommands:
...
#   kill-orphans    remove orphan teamwork/opencode processes (ppid == launchd)
...
#   wait-healthy    block until teamwork-server reports /health = 200
#   reset           stop + wipe Vite dep cache + truncate log sink + start
#   restart         alias for reset
#
# Variant (TEAMWORK_DEV_VARIANT):
#   electron   (default) pnpm --filter @teamwork/desktop dev:electron
...
#   3. Tauri webview       (target/debug/TeamWork-Dev)  <-- never /Applications/
#   4. Electron main+helpers (node_modules/electron/...Electron.app)
#   5. Vite                (node node_modules/.../vite)
#   6. orchestrator + teamwork-server + opencode + opencode-router
...
#   - ~/Library/Application Support/com.differentai.teamwork.dev/** (tokens,
#     workspaces registry, prefs). Use `reset-webview` for WebKit state.
#   - /Applications/TeamWork.app (prod build never targeted).
#
set -euo pipefail
...
DEV_LOG_FILE="${TEAMWORK_DEV_LOG_FILE:-$HOME/.teamwork/debug/teamwork-dev.log}"
PNPM_DEV_LOG="${TEAMWORK_PNPM_DEV_LOG:-/tmp/teamwork-test/pnpm-dev.log}"
PNPM_DEV_PID_FILE="${TEAMWORK_PNPM_DEV_PID:-/tmp/teamwork-test/pnpm-dev.pid}"
WAIT_HEALTHY_SECS="${TEAMWORK_WAIT_HEALTHY_SECS:-90}"
ELECTRON_CDP_PORT="${TEAMWORK_ELECTRON_REMOTE_DEBUG_PORT:-9823}"
...
DEV_VARIANT="${TEAMWORK_DEV_VARIANT:-electron}"
case "$DEV_VARIANT" in electron|tauri) ;; *)
  printf '[teamwork-debug] unknown TEAMWORK_DEV_VARIANT=%s (expected electron|tauri)\n' "$DEV_VARIANT" >&2
  exit 2
  ;;
esac
...
kill_by_pattern() {
  ...
  # webview (matched by its target/debug path, so prod TeamWork.app is safe).
  local pattern="$1"
  local pids
...
discover_teamwork_server_port() {
  ps -Ao command \
    | grep -E "(target/debug|apps/desktop/src-tauri/sidecars)/teamwork-server" \
    | grep -v grep \
    | grep -oE '\-\-port [0-9]+' \
    | head -1 \
    | awk '{print $2}' \
...
electron_renderer_stats() {
  ps -axo pid,ppid,pcpu,pmem,rss,command \
    | awk '/Electron Helper \(Renderer\)/ && /com\.differentai\.teamwork/ && !/awk/ && !/grep/ {print; found=1} END {exit found ? 0 : 1}' \
    || true
}
...
probe_electron_page_cdp() {
  node <<'NODE'
const port = process.env.TEAMWORK_ELECTRON_REMOTE_DEBUG_PORT || "9823";
...
snapshot() {
  ...
  ps -Ao pid,ppid,command | awk '/target\/debug\/TeamWork-Dev|node_modules\/electron\/dist\/Electron\.app\/Contents\/MacOS\/Electron|apps\/desktop\/scripts\/electron-dev\.mjs|target\/debug\/teamwork-server|target\/debug\/teamwork-orchestrator|target\/debug\/opencode( |\/)|target\/debug\/opencode-router|apps\/desktop\/src-tauri\/sidecars\/teamwork-server|apps\/desktop\/src-tauri\/sidecars\/teamwork-orchestrator|apps\/desktop\/src-tauri\/sidecars\/opencode( |\/)|apps\/desktop\/src-tauri\/sidecars\/opencode-router|vite|pnpm .*dev/ && !/awk/ && !/grep/' | sed -E 's#/Users/[^ ]*/#…/#g' | head -20

  echo
  echo "=== teamwork-server ==="
  local port
  port=$(discover_teamwork_server_port)
  if [[ -z "$port" ]]; then
    echo "  (no dev teamwork-server running)"
  else
  ...
  oc_port=$(ps -Ao command \
    | grep -E "(target/debug|apps/desktop/src-tauri/sidecars)/teamwork-orchestrator" \
    | grep -v grep \
  ...
  ps -Ao pid,ppid,command | awk '$2 == 1 && $3 ~ /teamwork-server|teamwork-orchestrator|opencode( |\/)|opencode-router/' | head
  ...
  if [[ -f "$DEV_LOG_FILE" ]]; then
    ...
    echo "  (no sink file yet — run the dev app with TEAMWORK_DEV_LOG_FILE set)"
  fi
}
...
kill_orphans() {
  local pids
  pids=$(ps -Ao pid,ppid,command | awk '$2 == 1 && $3 ~ /teamwork-server|teamwork-orchestrator|opencode( |\/)|opencode-router/ {print $1}')
  if [[ -z "$pids" ]]; then
    log "no orphans"
    return 0
  fi
...
diagnose_hang() {
  ...
  if [[ "$browser_json" == *"webSocketDebuggerUrl"* && "$target_json" == *"webSocketDebuggerUrl"* ]]; then
    if page_probe=$(TEAMWORK_ELECTRON_REMOTE_DEBUG_PORT="$ELECTRON_CDP_PORT" probe_electron_page_cdp 2>&1); then
  ...
  port=$(discover_teamwork_server_port)
  if [[ -n "$port" ]]; then
    echo "  teamwork-server port=$port"
    curl -sS --max-time 2 "http://127.0.0.1:$port/health" || echo "unreachable"
    echo
  else
    echo "  no teamwork-server port discovered"
  fi
  ...
  if [[ -n "$renderer_pid" && ( "$page_probe" == "failed" || "$high_cpu" == "true" ) ]]; then
    local sample_file="/tmp/teamwork-renderer-${renderer_pid}-$(date +%Y%m%dT%H%M%S).sample.txt"
    ...
    echo "  skipped (renderer responsive and CPU not high). Set TEAMWORK_FORCE_SAMPLE=1 not currently supported."
  else
...
stop() {
  ...
  kill_by_pattern "target/debug/TeamWork-Dev"
  ...
  # 5. teamwork-server / orchestrator / opencode / opencode-router for the
  ...
  kill_by_pattern "target/debug/teamwork-server"
  kill_by_pattern "target/debug/teamwork-orchestrator"
  kill_by_pattern "target/debug/opencode"
  kill_by_pattern "target/debug/opencode-router"
  kill_by_pattern "apps/desktop/src-tauri/sidecars/teamwork-server"
  kill_by_pattern "apps/desktop/src-tauri/sidecars/teamwork-orchestrator"
...
start() {
  ...
  case "$DEV_VARIANT" in
    electron)
      ...
      env TEAMWORK_DEV_LOG_FILE="$DEV_LOG_FILE" \
        nohup pnpm --filter @teamwork/desktop dev:electron >"$PNPM_DEV_LOG" 2>&1 &
      pid=$!
      ;;
    tauri)
...
wait_healthy() {
  ...
  while (( SECONDS < deadline )); do
    ...
    if [[ -n "$port" ]]; then
      ...
      if [[ "$code" == "200" ]]; then
        log "teamwork-server healthy on :$port"
        return 0
      fi
    fi
    sleep 1
  done
  log "teamwork-server did not become healthy within ${WAIT_HEALTHY_SECS}s" >&2
  return 1
}
...
reset_webview_state() {
  ...
  # the teamwork-workspaces.json registry or server-side tokens.
  local webkit_dir="$HOME/Library/WebKit/com.differentai.teamwork.dev"
