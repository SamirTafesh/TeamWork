...
# TeamWork
> TeamWorkは、Claude Cowork/Codex（デスクトップアプリ）のオープンソース代替です。

## 基本理念

- ローカルファースト、クラウド対応: TeamWorkはワンクリックであなたのマシン上で動作します。メッセージを即座に送信できます。
- コンポーザブル: デスクトップアプリ、WhatsApp/Slack/Telegramコネクタ、またはサーバー。用途に合ったものを使えます。ロックインなし。
- エジェクタブル: TeamWorkはOpenCodeで動いているため、OpenCodeでできることはすべてTeamWorkでも動作します（UIがなくても）。
- シェアリング・イズ・ケアリング: localhostでソロ作業を始め、必要に応じてリモート共有を明示的にオプトインできます。

<p align="center">
  <img src="../app-demo.gif" alt="TeamWork デモ" width="800" />
</p>

TeamWorkは、エージェントワークフローを再現可能なプロダクト化されたプロセスとして簡単にリリースできるように設計されています。

## 代替UI
- **TeamWork Orchestrator（CLIホスト）**: デスクトップUIなしでOpenCode + TeamWorkサーバーを実行します。
  - インストール: `npm install -g teamwork-orchestrator`
  - 実行: `teamwork start --workspace /path/to/workspace --approval auto`
  - ドキュメント: [apps/orchestrator/README.md](../apps/orchestrator/README.md)

## クイックスタート

デスクトップアプリを[teamworklabs.com/download](https://teamworklabs.com/download)からダウンロードするか、最新の[GitHubリリース](https://github.com/SamirTafesh/TeamWork/releases)を取得するか、以下の手順でソースからインストールしてください。

- macOSおよびLinux向けのダウンロードが直接利用可能です。
- Windowsへのアクセスは現在、[teamworklabs.com/pricing#windows-support](https://teamworklabs.com/pricing#windows-support)の有料サポートプランで提供されています。
- ホステッドTeamWork Cloudワーカーは、チェックアウト後にWebアプリから起動し、デスクトップアプリから`Add a worker` -> `Connect remote`で接続します。

## なぜTeamWorkか

現在のOpenCode向けCLIやGUIは開発者を中心に設計されています。つまり、ファイルの差分、ツール名、そしてCLIを公開しなければ拡張が難しい機能に焦点が当てられています。

TeamWorkは以下を目指して設計されています:

- **拡張可能**: スキルとOpenCodeプラグインはインストール可能なモジュールです。
- **監査可能**: 何が、いつ、なぜ起きたかを表示します。
- **権限管理**: 特権フローへのアクセスを制御します。
- **ローカル/リモート**: TeamWorkはローカルでもリモートサーバーへの接続でも動作します。
...
TeamWorkは現在 `apps/app`（UI）と `apps/desktop`（デスクトップシェル）に配置されています。
...
`pnpm dev` は自動的に `TEAMWORK_DEV_MODE=1` を有効にするため、デスクトップ開発では個人のグローバル設定/認証/データの代わりに分離されたOpenCode状態を使用します。
...
リポジトリのすべての `dev` エントリポイントは同じ開発モード分離にオプトインするため、ローカルテストでは一貫してTeamWork管理のOpenCode状態を使用します。
...
- **ホストモード**では、TeamWorkはローカルホストスタックを実行し、UIをそれに接続します。
  - デフォルトランタイム: `teamwork`（`teamwork-orchestrator` からインストール）。`opencode`、`teamwork-server`、およびオプションで `opencode-router` をオーケストレーションします。
  - フォールバックランタイム: `direct`。デスクトップアプリが直接 `opencode serve --hostname 127.0.0.1 --port <free-port>` を起動します。

プロジェクトフォルダを選択すると、TeamWorkはそのフォルダを使用してローカルでホストスタックを実行し、デスクトップUIを接続します。
...
プラグインはOpenCodeを拡張する**ネイティブ**な方法です。TeamWorkはスキルタブから `opencode.json` を読み書きして管理します。

- **プロジェクトスコープ**: `<workspace>/opencode.json`
- **グローバルスコープ**: `~/.config/opencode/opencode.json`（または `$XDG_CONFIG_HOME/opencode/opencode.json`）

`opencode.json` を手動で編集することもできます。TeamWorkはOpenCode CLIと同じ形式を使用します:
...
TeamWorkがWebKitGTKエラー（`Failed to create GBM buffer` など）で起動時にクラッシュする場合は、起動前にdmabufまたはコンポジティングを無効にしてください。以下のいずれかの環境変数フラグを試してください。
...
- TeamWorkはデフォルトでモデルの推論と機密ツールメタデータを非表示にします。
...
組織でのTeamWork利用に興味がありますか？ぜひお聞かせください — [ben@teamworklabs.com](mailto:ben@teamworklabs.com) までユースケースについてご連絡ください。
